...
配置文件：
  默认读脚本同目录的 config.json；不存在时回退到 .workbuddy\tmp\config.json。
  注意：配置里的相对路径（如 db_path）按【脚本所在目录】解析，不是按你的当前目录。
  这是刻意为之——保证从任何位置调用脚本，命中同一份配置和同一个数据库。
#>
param(
  [string]$ConfigPath = "",
  [string]$MasterKey = "",
...
# ---------- 解析配置（-Stop 也需要端口） ----------
if (-not $ConfigPath) {
  $cand = Join-Path $Root 'config.json'
  $test = Join-Path $Root '.workbuddy\tmp\config.json'
  if (Test-Path $cand) { $ConfigPath = $cand }
  elseif (Test-Path $test) {
    $ConfigPath = $test
    Write-Host "[i] 根目录没有 config.json，使用测试配置 $ConfigPath" -ForegroundColor Yellow
  }
  else { Die "找不到配置文件。请创建 $cand，或用 -ConfigPath 指定路径。" }
}
if (-not (Test-Path $ConfigPath)) { Die "配置文件不存在：$ConfigPath" }
$ConfigPath = (Resolve-Path $ConfigPath).Path

# 从 listen 字段提取端口（"127.0.0.1:18080" -> 18080），解析失败按默认 18080
$Port = 18080
try {
...
$cmdLine = '/c ""' + $Bin + '" -config "' + $ConfigPath + '"' +
           ' > "' + $outLog + '" 2> "' + $errLog + '""'
